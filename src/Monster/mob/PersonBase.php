...
abstract class PersonBase extends Human {
    ...
    public function isPlayersinRadius() {
        //$entities = $this->level->getChunkEntities($this->x, $this->z);
        /*$entities = $this->level->getNearbyEntities(new AxisAlignedBB($this->x - 12, $this->y - 12, $this->z - 12, $this->x + 12, $this->y + 12, $this->z + 12));
        $players = $this->level->getPlayers();
        $full_entities = $entities + $players;
        $full_entities = array_unique($full_entities);
        if((count($entities) + count($players)) > count($full_entities))
          return true;
        return false;*/
        if (isset(Monster::getInstance()->monster_list[Monster::getInstance()->getIdToName($this->getName())]) &&
                count(Monster::getInstance()->monster_list[Monster::getInstance()->getIdToName($this->getName())]) >= 1)
            return true;
        foreach ($this->level->getNearbyEntities($this->boundingBox->expandedCopy($this->target_find_radius, $this->target_find_radius, $this->target_find_radius), $this) as $player) {
            if ($player instanceof Player && $player->getGamemode() !== 3)
                return true;
        }
        return false;
    }
    ...
    public function followByWalking(Entity $target, float $xOffset = 0.0, float $yOffset = 0.0, float $zOffset = 0.0): void {
        ...
        if ($target !== null) {
            ...
            if ($xz_sq < $this->follow_range_sq) {
                ...
                $speed_factor = $this->getSpeed() - 0.04; // 건의로 너프된 이속
